...
struct
  ...
  let set_pwm_range pin range =
    ...
    match channel_of_pin pin with
    | Chan_0 -> ( Mem_map32.write regs pwm0_range range32 ; Time.delay_microseconds 10 )
    | Chan_1 -> ( Mem_map32.write regs pwm1_range range32 ; Time.delay_microseconds 10 )

  let get_pwm_range pin =
    (* get range only for the according channel *)
    Int32.to_int ( 
      match channel_of_pin pin with
      | Chan_0 -> Mem_map32.read regs pwm0_range
  ...
  let get_pulse_data pin =
    Int32.to_int (
      match channel_of_pin pin with
      | Chan_0 -> Mem_map32.read regs pwm0_data
      | Chan_1 -> Mem_map32.read regs pwm1_data
    )
  ...
  let set_pulse_data pin pulse_data =
    ...
    match channel_of_pin pin with
    | Chan_0 -> Mem_map32.write regs pwm0_data pd
    | Chan_1 -> Mem_map32.write regs pwm1_data pd
...
struct
  ...
  let set_divisor divisor =
    ...
    let pwm_control = Mem_map32.read Pwm.regs Pwm.control in      (* preserve pwm_control *)
    (* We need to stop pwm prior to stopping pwm clock in MS mode otherwise BUSY stays high. *)
    begin
      Mem_map32.write Pwm.regs Pwm.control (Int32.of_int 0);      (* Stop pwm *)
      (* Stop pwm clock before changing divisor. The delay after this does need to
      this big (95uS occasionally fails, 100uS OK), it's almost as though the BUSY
      flag is not working properly in balanced mode. Without the delay when DIV is
      adjusted the clock sometimes switches to very slow, once slow further DIV
      adjustments do nothing and it's difficult to get out of this mode. *)
      Mem_map32.write regs cntl (Int32.logor bcm_password 0x01l); (* Stop Clock *)
      Time.delay_microseconds 110;                                (* prevents clock going sloooow *)
      (while ((Int32.to_int (Mem_map32.read regs cntl) land 0x80) != 0) (* Wait for clock to be !BUSY *)
        do (Time.delay_microseconds 1) done; ()); 
      let clk = Int32.logor bcm_password (Int32.shift_left (Int32.of_int divisor) 12) in
      Mem_map32.write regs div clk;                               (* write new divisor *)
      Mem_map32.write regs cntl (Int32.logor bcm_password  0x11l);(* Start pwm clock *)
      Mem_map32.write Pwm.regs Pwm.control pwm_control;           (* restore pwm_control *)
      ()
    end

  let get_divisor () =
    ((Int32.to_int (Mem_map32.read regs div)) lsr 12)
